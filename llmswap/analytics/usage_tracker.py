...
class UsageTracker:
    ...
    def record_usage(
        ...
        self._update_daily_stats(
            timestamp,
            total_tokens,
            actual_cost or estimated_cost or 0,
            provider,
            success,
            response_time_ms,
            cache_hit,
    ...
    def _update_daily_stats(
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            if existing:
                ...
                try:
                    existing_avg_time = (
                        float(existing[6]) if existing[6] is not None else 0.0
                    )
                    if response_time_ms and existing_avg_time > 0:
                        new_avg_response_time = (
                            existing_avg_time + response_time_ms
                        ) / 2
                ...
                try:
                    new_cache_hit_rate = (
                        float(existing[7]) if existing[7] is not None else 0.0
                    )
                    existing_success_rate = (
                        float(existing[8]) if existing[8] is not None else 0.0
                    )
                    existing_queries = (
                        int(existing[1]) if existing[1] is not None else 0
                    )
                    new_success_rate = (
                        (existing_success_rate * existing_queries)
                        + (1 if success else 0)
                    ) / new_total_queries
                except (ValueError, TypeError):
                ...
                provider_stats = {
                    provider: {"queries": 1, "tokens": tokens, "cost": cost}
                }
    ...
    def get_usage_stats(self, days: int = 30) -> Dict[str, Any]:
        ...
        return {
            ...
            "totals": {
                ...
                "avg_cost_per_query": (
                    round(total_cost / total_queries, 4) if total_queries > 0 else 0
                ),
            },
    ...
    def get_cost_analysis(self) -> Dict[str, Any]:
        ...
        cheapest = (
            min(providers, key=lambda p: p["cost"])
            if len(providers) > 1
            else most_expensive
        )
        ...
        if len(providers) > 1 and cheapest["cost"] < most_expensive["cost"]:
            cost_per_query_expensive = (
                most_expensive["cost"] / most_expensive["queries"]
            )
        ...
        overall_cache_rate = sum(
            p["cache_hit_rate"] * p["queries"] for p in providers
        ) / sum(p["queries"] for p in providers)
        cache_savings_estimate = (
            stats["totals"]["cost"] * overall_cache_rate * 0.5
        )  # Rough estimate
        ...
        return {
            ...
            "recommendations": self._generate_cost_recommendations(
                providers, overall_cache_rate
            ),
            "analysis_date": datetime.now().isoformat(),
        }

    def _generate_cost_recommendations(
        self, providers: List[Dict], cache_rate: float
    ) -> List[str]:
        ...
        if len(providers) > 1:
            ...
            if cheapest["provider"] != most_expensive["provider"]:
                savings_pct = (
                    (
                        (most_expensive["cost"] / most_expensive["queries"])
                        - (cheapest["cost"] / cheapest["queries"])
                    )
                    / (most_expensive["cost"] / most_expensive["queries"])
                    * 100
                )
    ...
    def export_usage_report(
        self, output_file: str, format: str = "json", days: int = 30
    ):
                ...
                for provider in stats["provider_breakdown"]:
                    writer.writerow(
                        [
                            ...
                            provider["tokens"],
                            f"${provider['cost']:.4f}",
                            f"{provider['avg_response_time_ms']:.1f}",
    ...
    def clear_old_data(self, days_to_keep: int = 90):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            daily_cutoff = cutoff_date - timedelta(
                days=365
