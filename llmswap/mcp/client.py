...
class MCPClient:
    ...
    def _initialize(self) -> None:
        ...
        logger.info(f"MCP session initialized: {self._server_info.get('serverInfo', {}).get('name', 'unknown')}")
    ...
    def list_tools(self) -> List[Dict[str, Any]]:
        ...
        if response.is_error():
            raise MCPError(
                f"list_tools failed: {response.error}",
                details=response.error
            )
    ...
    def list_resources(self) -> List[Dict[str, Any]]:
        ...
        if response.is_error():
            raise MCPError(
                f"list_resources failed: {response.error}",
                details=response.error
            )
    ...
    def is_connected(self) -> bool:
        ...
        return (
            ...
            self.transport.is_connected() and
            self._initialized
        )
