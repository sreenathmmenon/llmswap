...
class SSETransport(BaseTransport):
        ...
        try:
            ...
            with urllib.request.urlopen(request, timeout=self.timeout) as response:
                if response.status != 200:
                    raise MCPTransportError(f"HTTP {response.status}: {response.reason}")
    ...
    def receive_message(self, timeout: Optional[float] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.debug(f"Received message via SSE: {message.get('id', 'notification')}")
            
            return message
            
        except queue.Empty:
            raise MCPTimeoutError(
                f"Timeout waiting for SSE message ({timeout}s)",
                retry_after=timeout
            )
    ...
    def is_healthy(self) -> bool:
        ...
        return (
            self._connected and
            self._running and
            self._sse_thread is not None and
            self._sse_thread.is_alive()
        )
