...
class ConnectionPool:
    ...
    def acquire(self, timeout: Optional[float] = None) -> MCPClient:
        ...
        try:
            ...
            logger.debug(f"Acquired connection for {self.server_name} in {acquire_time:.3f}s")
        ...
        with self._lock:
            ...
            if conn.is_connected():
                try:
                    ...
                    logger.debug(f"Pool full, closing excess connection for {self.server_name}")
                    conn.close()
                    self._total_created -= 1
            else:
    ...
    def _create_new_connection(self) -> MCPClient:
        ...
        self._total_created += 1
        logger.debug(f"Created new connection for {self.server_name} (total: {self._total_created})")
        return conn
    ...
    def _record_acquire_time(self, acquire_time: float) -> None:
        ...
        if self._acquire_times:
            self._metrics.average_acquire_time = sum(self._acquire_times) / len(self._acquire_times)
