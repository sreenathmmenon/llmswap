...
class CleanUI:
    ...
    def header(self, provider: str, model: Optional[str] = None):
        ...
        print(f"  {self.c('Natural language interface • Type', Colors.DIM)} {self.c('help', Colors.YELLOW)} {self.c('for commands', Colors.DIM)}")
        print("=" * 70)
        print()
    
    def connection_status(self, server_type: str, tools_count: int, tools_names: list = None):
        """Show connection status"""
        print(f"{self.c('✓', Colors.GREEN)} Connected to {self.c(server_type, Colors.BOLD)} MCP server")
        
        tools_text = f"{tools_count} tool{'s' if tools_count != 1 else ''}"
        if tools_names and len(tools_names) <= 3:
            tools_list = ", ".join(tools_names)
            print(f"{self.c('✓', Colors.GREEN)} {tools_text} available: {self.c(tools_list, Colors.CYAN)}")
        else:
    ...
    def user_prompt(self) -> str:
        ...
        print(self.c("│ " + " " * 66 + " │", Colors.BRIGHT_BLACK))  # Empty line for input
        print(self.c("╰" + "─" * 68 + "╯", Colors.BRIGHT_BLACK))
    ...
    def tool_execution(self, tool_name: str, duration: Optional[float] = None):
        ...
        if duration is not None:
            print(f"  {self.c('→', Colors.BRIGHT_BLACK)} {self.c(tool_name, Colors.YELLOW)} {self.c('✓', Colors.GREEN)} {self.c(f'({duration:.2f}s)', Colors.DIM)}")
        else:
            print(f"  {self.c('→', Colors.BRIGHT_BLACK)} {self.c(tool_name, Colors.YELLOW)} {self.c('...', Colors.DIM)}", flush=True)
        print()
    ...
    def assistant_response(self, content: str):
        ...
        for line in content.split('\n'):
            # Handle empty lines
            if not line.strip():
                print(self.c("│", Colors.BRIGHT_BLACK) + " " * 68 + self.c("│", Colors.BRIGHT_BLACK))
                continue
            ...
            if len(line) <= 66:
                ...
                print(self.c("│ ", Colors.BRIGHT_BLACK) + padded_line + self.c(" │", Colors.BRIGHT_BLACK))
            else:
                # Simple word wrap
                words = line.split(' ')
                current_line = ""
    ...
    def error(self, message: str):
        """Show error message"""
        print(f"{self.c('✗', Colors.RED)} {self.c('Error:', Colors.BOLD + Colors.RED)} {message}")
        print()
    
    def warning(self, message: str):
        """Show warning message"""
        print(f"{self.c('⚠', Colors.YELLOW)} {self.c('Warning:', Colors.BOLD + Colors.YELLOW)} {message}")
        print()
