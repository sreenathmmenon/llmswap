    ...
    def __init__(
        ...
        try:
            if command:
                ...
                self.llm_client.add_mcp_server(
                    self.server_name,
                    command=command
                )
            elif url:
        ...
        while True:
            try:
                ...
                if user_input.lower() == "status":
                    self.ui.connection_status(self.server_type, len(self.tools), [t.get('name') for t in self.tools[:5]])
                    continue
                        ...
                        for tool_call in response.tool_calls:
                            try:
                                ...
                                if tool_name in self.llm_client._mcp_tools:
                                    ...
                                    # Tool not found
                                    executed_results.append({"content": f"Error: Tool '{tool_name}' not found"})
                                    if not self.quiet:
...
Environment Variables:
  ...
  XAI_API_KEY        - API key for X.AI Grok
    ...
    parser.add_argument(
        '--no-color',
        action='store_true',
        help='Disable colors and fancy UI'
    )
