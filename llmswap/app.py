...
def cmd_ask(args):
    ...
    try:
        client = LLMClient(
            provider=args.provider or "auto", cache_enabled=not args.no_cache
        )
...
"""
            ...
            if any(keyword in args.question.lower() for keyword in learning_keywords):
                print(
                    "💡 Detected learning question. Try --teach or --age for tailored explanation"
                )
...
def _build_context_instruction(chat_context):
    ...
    if chat_context.age:
        ...
        return level_instructions.get(
            chat_context.level, level_instructions["intermediate"]
        )
    return ""
...
def cmd_chat(args):
    ...
    try:
        ...
        print(f"🤖 llmswap Interactive Chat v{__import__('llmswap').__version__}")
        ...
        if chat_context:
            print(
                "          /age <number>, /audience <type>, /level <level>, /reset-context"
            )
            ...
            if workspace_dir:
                ...
                for ws in all_workspaces:
                    if ws["workspace_id"] == workspace_id:
                        ...
                        print(
                            f"📊 Learnings: {data['statistics']['learnings_count']} tracked"
                        )
                        break
            else:
            ...
            try:
                ...
                if user_input.startswith("/"):
                    ...
                    if command in ["quit", "exit", "q"]:
                        ...
                        print(
                            "  /audience <type>   - Set audience (e.g., /audience student)"
                        )
                        print(
                            "  /level <level>     - Set level (beginner/intermediate/advanced)"
                        )
                        ...
                        print(
                            "  • Provider switching starts fresh conversations (privacy protection)"
                        )
                        ...
                        if new_provider in available:
                            try:
                                ...
                                print(
                                    "   ⚠️  You'll start a completely fresh conversation"
                                )
                                    ...
                                    if result["success"]:
                                        ...
                                        if result.get("previous_session"):
                                            ...
                                            if (
                                                ...
                                                print(
                                                    f"   Tokens: {prev['session_tokens']}"
                                                )
                                                print(
                                                    f"   Cost: ~${prev['session_cost']:.4f} (est.)"
                                                )
                            ...
                            print(
                                f"❌ Invalid provider. Choose from: {', '.join(available)}"
                            )
                        continue
                        ...
                        print(
                            f"  Session Cost: ~${session_info['session_cost']:.6f} (estimated)"
                        )
                    ...
                    # Age/Audience context commands
                        ...
                        try:
                            ...
                            print(
                                "❌ Usage: /audience <type> (e.g., /audience student)"
                            )
                        continue
                        ...
                        try:
                            ...
                            print(
                                "❌ Usage: /level <level> (beginner/intermediate/advanced)"
                            )
                        continue
                ...
                if (
                    hasattr(client, "_chat_context")
                    and client._chat_context.has_context()
                ):
...
def cmd_logs(args):
    ...
    if not args.analyze:
        ...
        print(
            "  llmswap logs --analyze 'app.log,api.log' --request-id req-456 --correlate"
        )
        return 1
    ...
    for log_file in log_files:
        try:
            with open(log_file, "r", encoding="utf-8", errors="ignore") as f:
                content = f.read(
                    args.max_lines * 200
                )  # Estimate ~200 chars per log line
                all_content += f"\n=== {log_file} ===\n{content}\n"
    ...
    try:
        ...
        if args.format == "json":
            ...
            for provider, data in comparison["comparison"].items():
                print(
                    f"{provider},{data.get('model','')},{data['total_cost']},{data.get('input_cost',0)},{data.get('output_cost',0)},{data.get('confidence','')}"
                )
        else:  # table format (default)
            print(
                f"💰 Provider Cost Comparison ({args.input_tokens} input + {args.output_tokens} output tokens)\n⚠️  Estimates only - actual billing may vary"
            )
            print("=" * 80)
            print(
                f"{'Provider':<12} {'Model':<20} {'Total Cost':<12} {'Savings':<12} {'Confidence'}"
            )
            print("-" * 80)
            ...
            for provider, data in providers:
                savings = (
                    ((max_cost - data["total_cost"]) / max_cost * 100)
                    if max_cost > 0
                    else 0
                )
                ...
                print(
                    f"{provider:<12} {data.get('model','')[:19]:<20} {cost_str:<12} {savings_str:<12} {data.get('confidence','')}"
                )

            print("-" * 80)
            print(
                f"🏆 Cheapest: {comparison['cheapest']} (~${comparison['cheapest_cost']:.6f})"
            )
            print(
                f"💸 Most Expensive: {comparison['most_expensive']} (~${comparison['most_expensive_cost']:.6f})"
            )
...
def cmd_usage(args):
    ...
    try:
        ...
        if not stats:
            print(
                "No usage data available. Make some queries with analytics enabled first."
            )
            return 1
        ...
        if args.format == "json":
            ...
            for day in stats["daily_breakdown"]:
                for provider, data in day["provider_breakdown"].items():
                    print(
                        f"{day['date']},{data['queries']},{data['tokens']},{data['cost']},{provider}"
                    )
        else:  # table format (default)
            print("📊 Usage Statistics")
            print("=" * 60)
            print(
                f"Period: {stats['period']['days']} days ({stats['period']['start_date']} to {stats['period']['end_date']})"
            )
            print()
            ...
            if stats["provider_breakdown"]:
                print("🏢 By Provider:")
                print(
                    f"{'Provider':<12} {'Queries':<8} {'Tokens':<10} {'Cost':<10} {'Avg Response'}"
                )
                print("-" * 55)
                for provider in stats["provider_breakdown"]:
                    tokens_str = (
                        f"{provider['tokens']:,}" if provider["tokens"] else "0"
                    )
                    cost_str = f"${provider['cost']:.4f}"
                    response_str = f"{provider['avg_response_time_ms']:.0f}ms"
                    print(
                        f"{provider['provider']:<12} {provider['queries']:<8} {tokens_str:<10} {cost_str:<10} {response_str}"
                    )
            ...
            try:
                ...
                if confirm == "y" or confirm == "yes":
                    ...
                    result = subprocess.run(
                        generated_code, shell=True, capture_output=True, text=True
                    )
                    if result.stdout:
                        print("Output:")
                        print(result.stdout)
...
def cmd_costs(args):
    ...
    try:
        ...
        if not analysis:
            print(
                "No cost data available. Make some queries with analytics enabled first."
            )
            return 1
        ...
        if args.format == "json":
            ...
            print(
                f"  Provider Savings:    ${opt.get('potential_provider_savings', 0):.2f}"
            )
            print(f"  Cache Savings Est:   ${opt.get('cache_savings_estimate', 0):.2f}")
            print(
                f"  Cache Hit Rate:      {opt.get('overall_cache_hit_rate', 0)*100:.1f}%"
            )
            print()
    ...
    if hasattr(args, "verify") and args.verify:
        ...
        if "error" in verification_result:
            ...
            if "available_providers" in verification_result:
                print(
                    f"Available providers: {', '.join(verification_result['available_providers'])}"
                )
            return 1
    ...
    try:
        ...
        if args.format == "json":
            ...
            try:
                ...
                print(f"{'='*60}")
                print(
                    f"{'Provider':<12} {'Default Model':<25} {'Status':<15} {'Issue'}"
                )
                print("-" * 60)
            ...
            print(
                f"💡 Use: llmswap config set provider.models.<provider> <model> to change defaults"
            )
...
def cmd_workspace(args):
    ...
    if args.workspace_action == "init":
        ...
        try:
            ...
            print(
                f"💾 Location: ~/.llmswap/workspaces/{workspace_data['workspace_id']}/"
            )
            print(f"\nCreated files:")
        ...
        for ws in workspaces:
            table_data.append(
                [ws["project_name"], ws["project_path"], ws["last_accessed"][:10]]
            )
...
Examples:
  ...
  llmswap costs
    ...
    parser.add_argument(
        "--quiet", "-q", action="store_true", help="Suppress tips and extra output"
    )
    parser.add_argument(
        "--version",
        action="version",
    ...
    ask_parser.add_argument(
        "--mentor",
        choices=["teacher", "mentor", "tutor", "developer", "professor", "buddy"],
        help="Choose teaching style",
    )
    ask_parser.add_argument("--alias", type=str, help="Custom name for your mentor")

    # Age-appropriate explanations
    ask_parser.add_argument(
        "--age", type=int, help="Target age for explanation (e.g., 10, 25, 50)"
    )
    ask_parser.add_argument(
        "--audience",
        type=str,
    ...
    chat_parser.add_argument(
        "--teach", action="store_true", help="Start in teaching mode"
    )
    chat_parser.add_argument(
        "--mentor",
        choices=["teacher", "mentor", "tutor", "developer", "professor", "buddy"],
    ...
    chat_parser.add_argument("--alias", type=str, help="Custom name for your mentor")

    # Age-appropriate explanations for chat
    chat_parser.add_argument(
        "--age", type=int, help="Set default age context for explanations"
    )
    chat_parser.add_argument(
        "--audience", type=str, help="Set default audience context"
    )
    chat_parser.add_argument(
        "--level",
        choices=["beginner", "intermediate", "advanced"],
    ...
    debug_parser.add_argument(
        "--error", "-e", required=True, help="Error message to analyze"
    )

    # logs command - Advanced log analysis
    logs_parser = subparsers.add_parser("logs", help="Advanced AI-powered log analysis")
    logs_parser.add_argument(
        "--analyze", help="Log file(s) to analyze (comma-separated for multiple)"
    )

    # Time-based filtering
    logs_parser.add_argument(
        "--since", help='Time range: "2h ago", "10m ago", "1d ago"'
    )
    logs_parser.add_argument(
        "--between",
        help='Time range: "10:00-14:00" or "2025-01-17 10:00 to 2025-01-17 14:00"',
    )
    logs_parser.add_argument(
        "--timerange", help='Specific time range: "9AM-11AM", "last 4 hours"'
    )
    ...
    logs_parser.add_argument(
        "--terms", help='Search terms (comma-separated): "error,timeout,failed"'
    )
    logs_parser.add_argument("--exclude-terms", help="Exclude terms (comma-separated)")
    logs_parser.add_argument(
        "--level", help='Log level filter: "error", "warn", "info", "debug"'
    )
    ...
    logs_parser.add_argument(
        "--correlate", action="store_true", help="Enable cross-log correlation analysis"
    )

    # Output options
    logs_parser.add_argument(
        "--format",
        choices=["summary", "detailed", "timeline"],
        default="summary",
    ...
    generate_parser = subparsers.add_parser(
        "generate", help="Generate commands or code from natural language"
    )
    generate_parser.add_argument(
        "description", help="Natural language description of what you want to generate"
    )
    generate_parser.add_argument(
        "--language",
        "-l",
    ...
    generate_parser.add_argument("--save", "-s", help="Save generated code to file")

    # costs command - Cost analysis and optimization
    costs_parser = subparsers.add_parser(
        "costs", help="Analyze costs and get optimization suggestions"
    )
    ...
    config_parser.add_argument(
        "key", nargs="?", help="Configuration key (e.g., provider.default)"
    )

    # providers command - Show provider status table
    providers_parser = subparsers.add_parser(
        "providers", help="Show all providers and their status"
    )
    ...
    providers_parser.add_argument(
        "--provider", type=str, help="Verify specific provider only"
    )
    providers_parser.add_argument(
        "--timeout",
        type=int,
    ...
    workspace_parser = subparsers.add_parser(
        "workspace", help="Manage project workspaces"
    )
    ...
    web_parser = subparsers.add_parser(
        "web", help="Start local web UI for comparing models"
    )
